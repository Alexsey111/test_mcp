...
Простой тестовый бэкенд на FastAPI, который возвращает текущее время сервера, умеет конвертировать время между часовыми поясами.
...
Приложение разворачивается на порту `8001` сервера (внутри контейнера порт `8000`).
